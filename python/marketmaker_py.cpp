...
MM_insert(pyMM* self, PyObject* args, PyObject* kwds)
{
    ...
    if(!PyArg_ParseTupleAndKeywords(args, kwds, "fk|b", kwlist, &limit, &size, &no_order_cb))
    {
...
MM_New(PyTypeObject* type, PyObject* args, PyObject* kwds)
{
    ...
    if(!PyArg_ParseTupleAndKeywords(args, kwds, "O|OO:__new__", kwlist, &cb, &start, &stop))
    {
    ...
    if(self != NULL){
        try{
            mm = new MarketMaker_Py( StartFuncWrap(start),
                                     StopFuncWrap(stop), 
                                     ExecCallbackWrap(cb) );
            if(!mm)
...
PyTypeObject pyMM_type = {
    ...
    0,
    Py_TPFLAGS_DEFAULT,
    "MarketMaker: 'autonomous' agents that provide (necessary) liquity to orderbook\n\n"
    "  start callback ::  (float price, float tick) -> void  :: function that is called on start\n"
    "  stop callback  ::  (void) -> void  :: function that is called on stop\n"
    "  exec callback  ::  (int msg, int id, float price, int size) -> void  :: "
    "function that is called on order execution\n"  ,
    0,
    0,
    0,
    0,
