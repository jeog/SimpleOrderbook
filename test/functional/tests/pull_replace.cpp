...
int
TEST_orders_info_pull_ASYNC_1(FullInterface *orderbook, std::ostream& out)
{
    auto conv = [&](double d){ return orderbook->price_to_tick(d); };
    ...
    std::vector<std::pair<std::future<id_type>, double>> FVEC;


    for(double b = beg ; b < end; b = conv(b+incr) ){
        FVEC.emplace_back(orderbook->insert_limit_order_async(false, b, sz), conv(b));
        count++;
    }
    id_type stop_id = orderbook->insert_stop_order_async(true, beg, sz, callback).get();

    vector<pair<id_type, double>> ids;
    for(auto& fut : FVEC)
        ids.push_back( std::make_pair(fut.first.get(), fut.second) );
    ...
    good_pull = orderbook->pull_order_async(ids[0].first).get();
    if( !good_pull ){
        return 3;
    }
    out<< "PULL: " << ids[0].first << " "<< good_pull << endl;
    ...
    stop_id = orderbook->insert_stop_order_async( false, conv(end-2*incr),
                                            conv(end-incr),  sz, callback).get();
    ...
    good_pull = orderbook->pull_order_async(stop_id).get();
    if( !good_pull ){
        return 9;
    }
    out<< "PULL: " << stop_id << " "<< good_pull << endl;
...
int
TEST_replace_order_ASYNC_1(FullInterface *orderbook, std::ostream& out)
{
    auto conv = [&](double d){ return orderbook->price_to_tick(d); };
    ...
    id_type id1 = orderbook->insert_limit_order_async(true, beg, sz, ecb).get();
    ids[id1] = id1;
    id_type id2 = orderbook->insert_limit_order(true, conv(beg+2*incr), sz, ecb);
    ids[id2] = id2;
    id_type id3 = orderbook->insert_limit_order_async(true, conv(beg+4*incr), sz, ecb).get();
    ids[id3] = id3;
    id_type id4 = orderbook->insert_stop_order_async( false, conv(beg+4*incr),
                                                conv(beg+incr), sz, ecb).get();
    ids[id4] = id4;
    // beg + 4  L 100     S 100
    // beg + 2  L 100
    // beg + 0  L 100
    ...
    orderbook->insert_market_order_async(false, static_cast<int>(sz/2), ecb).wait();
    ...
    auto aot = AdvancedOrderTicketOTO::build_limit(true, beg, sz);
    id_type id9 = orderbook->insert_market_order_async(true, sz, ecb, aot).get();
    tvol += sz;
    // ecb should have already handle order id change
    // beg + 3
    // beg + 2            L 200
